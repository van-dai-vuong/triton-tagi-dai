...
Originally fused Triton kernels; this is a pure-PyTorch port with identical
math, running on CPU (and CUDA/MPS if available).
"""
...
def compute_innovation(y, y_pred_mu, y_pred_var, sigma_v):
    ...
    if y_pred_mu.shape[-1] == 2 * y.shape[-1]:
        return _innovation_heteros(y, y_pred_mu, y_pred_var)

    Sy = y_pred_var + sigma_v**2
    delta_mu = (y - y_pred_mu) / Sy
    delta_var = -1.0 / Sy
    return delta_mu, delta_var


def _innovation_heteros(y, y_pred_mu, y_pred_var):
    """AGVI heteroscedastic output update — port of cuTAGI's CUDA
    ...
    the "overfit_mu" behavior — which is what the cuTAGI regression-heteros
    example (``cuda=True``) actually runs.
    """
    K = y.shape[-1]
    lead = y.shape[:-1]

    # Interleaved 2K output → (..., K, 2): [..., 0] = Z (mean), [..., 1] = V̄²_tilde.
    ym = y_pred_mu.reshape(*lead, K, 2)
    yS = y_pred_var.reshape(*lead, K, 2)

    mu_a_col = ym[..., 0]
    var_a_col = yS[..., 0]

    # V2_bar_tilde moments. For EvenExp, jcb = μ_a, so cov_V2_bar_tilde = μ.
    mu_v2_bar_tilde = ym[..., 1]
    var_v2_bar_tilde = yS[..., 1]
    ...
    zero = torch.zeros_like(obs_diff)
    delta_mu_col = torch.where(z_ok, obs_diff / var_a_col, zero)
    delta_var_col = torch.where(z_ok, -1.0 / var_sum, zero)
    ...
    delta_mu = torch.stack([delta_mu_col, delta_mu_v2], dim=-1).reshape(y_pred_mu.shape)
    delta_var = torch.stack([delta_var_col, delta_var_v2], dim=-1).reshape(y_pred_var.shape)
    return delta_mu, delta_var
