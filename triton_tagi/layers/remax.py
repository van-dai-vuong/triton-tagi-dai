...
_EPS = 1e-6  # match cuTAGI's floor
_INV_SQRT_2PI = 0.3989422804014327
_INV_SQRT_2 = 0.7071067811865475


# ======================================================================
#  Remax (MixtureReLU + log-normal path, cuTAGI parity)
# ======================================================================


def triton_remax(mu_z: Tensor, var_z: Tensor):
    """
    Compute Remax moments and Jacobian (pure-PyTorch port, row-wise over K).
    ...
    alpha = mu_z / std_z
    pdf_alpha = _INV_SQRT_2PI * torch.exp(-0.5 * alpha * alpha)
    cdf_alpha = 0.5 + 0.5 * torch.erf(alpha * _INV_SQRT_2)

    mu_m = (mu_z * cdf_alpha + std_z * pdf_alpha).clamp_min(_EPS)
    var_m = (
        -mu_m * mu_m
        + 2.0 * mu_m * mu_z
    ...
    # ── 2. Sum moments M̃ = Σ M_k (row-wise, broadcast back over K) ──
    mu_mt = mu_m.sum(dim=-1, keepdim=True).clamp_min(_EPS)
    var_mt = var_m.sum(dim=-1, keepdim=True).clamp_min(_EPS)
    ...
    var_log_a = (var_log_m + var_log_mt - 2.0 * cov_log_m_mt).clamp_min(0.0)

    # ── 6. Remax probabilities (renormalized) ──
    mu_a_raw = torch.exp(mu_log_a + 0.5 * var_log_a).clamp_min(_EPS)
    sum_mu_a = mu_a_raw.sum(dim=-1, keepdim=True).clamp_min(_EPS)
    ...
    cs_bound = torch.sqrt(var_a * var_z)
    cov_a_z = torch.minimum(cs_bound, cov_a_m / jcb_m.clamp_min(_EPS))
    J = cov_a_z / var_z
