...
GPU / Triton
------------
The graph runs on the GPU: every :class:`GaussianTensor` places its moments on
:data:`_DEFAULT_DEVICE` (CUDA by default, like the rest of ``triton_tagi``), and
the element-wise moment math this engine performs directly — Gaussian add /
multiply, the ``tanh`` / ``sigmoid`` activation moments, the activation-Jacobian
chain rule, and the observation innovation — is evaluated by the flat Triton
kernels defined below. Combined with the reused layer kernels above, the whole
graph (including the LSTM) executes on the GPU with no CPU fallback. Use
:func:`set_default_device` to override the placement.
...
# ======================================================================
#
#  Like the rest of ``triton_tagi`` (whose layers default to ``device="cuda"``),
#  the autocov graph runs on the GPU: every :class:`GaussianTensor` moves its
#  moments onto :data:`_DEFAULT_DEVICE`, and the moment math below is evaluated
#  by Triton kernels. Fall back to CPU only when no CUDA device is visible, so
#  the module still imports on a CPU-only host (the Triton kernels themselves
#  require a GPU at call time).

_DEFAULT_DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def set_default_device(device) -> None:
    """Set the device new :class:`GaussianTensor` leaves / layers are placed on."""
    global _DEFAULT_DEVICE
    _DEFAULT_DEVICE = torch.device(device)


def get_default_device() -> torch.device:
    """Return the device new :class:`GaussianTensor` leaves / layers use."""
...
# ======================================================================
#
#  The heavy ops (Linear/Conv2D/BatchNorm2D, ReLU, Remax, the capped parameter
#  update) already run through the library's Triton kernels. The remaining
#  moment math this engine performs directly — Gaussian add / multiply, the
#  tanh / sigmoid activation moments, the activation-Jacobian chain rule, and
#  the observation innovation — is evaluated here by flat element-wise Triton
#  kernels, so the whole graph (in particular the LSTM, built from ``*`` / ``+``
#  / ``sigmoid`` / ``tanh``) executes on the GPU. Every kernel follows the same
#  1-D grid / masked-load pattern as ``layers.relu`` and ``update.parameters``.

_BLOCK = 1024


def _grid(n: int):
    return (triton.cdiv(n, _BLOCK),)


@triton.jit
def _gauss_add_kernel(mx_ptr, Sx_ptr, my_ptr, Sy_ptr, mo_ptr, So_ptr, n, BLOCK: tl.constexpr):
    """S = X + Y (independent):  mu = mx + my,  var = Sx + Sy."""
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    mx = tl.load(mx_ptr + offs, mask=mask, other=0.0)
    Sx = tl.load(Sx_ptr + offs, mask=mask, other=0.0)
    my = tl.load(my_ptr + offs, mask=mask, other=0.0)
    Sy = tl.load(Sy_ptr + offs, mask=mask, other=0.0)
    tl.store(mo_ptr + offs, mx + my, mask=mask)
    tl.store(So_ptr + offs, Sx + Sy, mask=mask)


@triton.jit
def _gauss_mul_kernel(mx_ptr, Sx_ptr, my_ptr, Sy_ptr, mo_ptr, So_ptr, n, BLOCK: tl.constexpr):
    """Z = X * Y (element-wise, independent):
    mu  = mx*my
    var = Sx*Sy + Sx*my^2 + mx^2*Sy
    """
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    mx = tl.load(mx_ptr + offs, mask=mask, other=0.0)
    ...
    Sy = tl.load(Sy_ptr + offs, mask=mask, other=0.0)
    tl.store(mo_ptr + offs, mx * my, mask=mask)
    tl.store(So_ptr + offs, Sx * Sy + Sx * my * my + mx * mx * Sy, mask=mask)


@triton.jit
def _scale_delta_kernel(g_ptr, dmu_ptr, dvar_ptr, omu_ptr, ovar_ptr, n, BLOCK: tl.constexpr):
    """Chain a normalized innovation through a scalar gain ``g``:
    out_dmu = g * dmu,  out_dvar = g^2 * dvar.

    Serves both the multiply backward (g = the other operand's mean) and the
    generic activation backward (g = the activation Jacobian).
    """
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    g = tl.load(g_ptr + offs, mask=mask, other=0.0)
    dmu = tl.load(dmu_ptr + offs, mask=mask, other=0.0)
    dvar = tl.load(dvar_ptr + offs, mask=mask, other=0.0)
    tl.store(omu_ptr + offs, g * dmu, mask=mask)
    tl.store(ovar_ptr + offs, g * g * dvar, mask=mask)


@triton.jit
def _tanh_kernel(mz_ptr, Sz_ptr, mu_ptr, var_ptr, jcb_ptr, n, BLOCK: tl.constexpr):
    """First-order (linearized) Gaussian moments of tanh.

    mu = tanh(mz),  jcb = 1 - mu^2,  var = jcb^2 * Sz.
    tanh(x) is evaluated as 2*sigmoid(2x) - 1 (numerically stable via exp).
    """
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    mz = tl.load(mz_ptr + offs, mask=mask, other=0.0)
    Sz = tl.load(Sz_ptr + offs, mask=mask, other=0.0)
    mu = 2.0 / (1.0 + tl.exp(-2.0 * mz)) - 1.0
    jcb = 1.0 - mu * mu
    var = tl.maximum(jcb * jcb * Sz, 0.0)
    tl.store(mu_ptr + offs, mu, mask=mask)
    tl.store(var_ptr + offs, var, mask=mask)
    tl.store(jcb_ptr + offs, jcb, mask=mask)


@triton.jit
def _sigmoid_kernel(mz_ptr, Sz_ptr, mu_ptr, var_ptr, jcb_ptr, n, BLOCK: tl.constexpr):
    """First-order (linearized) Gaussian moments of the logistic sigmoid.

    mu = sigmoid(mz),  jcb = mu*(1 - mu),  var = jcb^2 * Sz.
    """
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    mz = tl.load(mz_ptr + offs, mask=mask, other=0.0)
    Sz = tl.load(Sz_ptr + offs, mask=mask, other=0.0)
    mu = 1.0 / (1.0 + tl.exp(-mz))
    jcb = mu * (1.0 - mu)
    var = tl.maximum(jcb * jcb * Sz, 0.0)
    tl.store(mu_ptr + offs, mu, mask=mask)
    tl.store(var_ptr + offs, var, mask=mask)
...
@triton.jit
def _innovation_kernel(
    y_ptr, mu_ptr, var_ptr, dmu_ptr, dvar_ptr, var_v, div_eps, n, BLOCK: tl.constexpr
):
    """Repo's normalized observation innovation for ``y = z + v``:
    var_y = max(var + var_v, div_eps)
    dmu   = (y - mu) / var_y
    dvar  = -1        / var_y
    """
    offs = tl.program_id(0) * BLOCK + tl.arange(0, BLOCK)
    ...
    mu = tl.load(mu_ptr + offs, mask=mask, other=0.0)
    var = tl.load(var_ptr + offs, mask=mask, other=0.0)
    var_y = tl.maximum(var + var_v, div_eps)
    tl.store(dmu_ptr + offs, (y - mu) / var_y, mask=mask)
    tl.store(dvar_ptr + offs, -1.0 / var_y, mask=mask)


def _gauss_add(mx, Sx, my, Sy):
    """Gaussian add moments (Triton). Returns (mu, var), shaped like ``mx``."""
    shape = mx.shape
    mx, Sx = mx.reshape(-1), Sx.reshape(-1)
    my, Sy = my.reshape(-1), Sy.reshape(-1)
    mo, So = torch.empty_like(mx), torch.empty_like(mx)
    n = mx.numel()
    _gauss_add_kernel[_grid(n)](mx, Sx, my, Sy, mo, So, n, BLOCK=_BLOCK)
    return mo.reshape(shape), So.reshape(shape)


def _gauss_mul(mx, Sx, my, Sy):
    """Gaussian element-wise product moments (Triton). Returns (mu, var)."""
    shape = mx.shape
    mx, Sx = mx.reshape(-1), Sx.reshape(-1)
    my, Sy = my.reshape(-1), Sy.reshape(-1)
    mo, So = torch.empty_like(mx), torch.empty_like(mx)
    n = mx.numel()
    _gauss_mul_kernel[_grid(n)](mx, Sx, my, Sy, mo, So, n, BLOCK=_BLOCK)
    return mo.reshape(shape), So.reshape(shape)


def _scale_delta(g, dmu, dvar):
    """Chain (dmu, dvar) through gain ``g``: (g·dmu, g²·dvar) (Triton)."""
    shape = dmu.shape
    g, dmu, dvar = g.reshape(-1), dmu.reshape(-1), dvar.reshape(-1)
    omu, ovar = torch.empty_like(dmu), torch.empty_like(dmu)
    n = dmu.numel()
    _scale_delta_kernel[_grid(n)](g, dmu, dvar, omu, ovar, n, BLOCK=_BLOCK)
    return omu.reshape(shape), ovar.reshape(shape)


def _tanh_moments(mz, Sz):
    """First-order (linearized) Gaussian moments of tanh (Triton)."""
    shape = mz.shape
    mz, Sz = mz.reshape(-1), Sz.reshape(-1)
    mu, var, jcb = torch.empty_like(mz), torch.empty_like(mz), torch.empty_like(mz)
    n = mz.numel()
    _tanh_kernel[_grid(n)](mz, Sz, mu, var, jcb, n, BLOCK=_BLOCK)
    return mu.reshape(shape), var.reshape(shape), jcb.reshape(shape)


def _sigmoid_moments(mz, Sz):
    """First-order (linearized) Gaussian moments of the logistic sigmoid (Triton)."""
    shape = mz.shape
    mz, Sz = mz.reshape(-1), Sz.reshape(-1)
    mu, var, jcb = torch.empty_like(mz), torch.empty_like(mz), torch.empty_like(mz)
    n = mz.numel()
    _sigmoid_kernel[_grid(n)](mz, Sz, mu, var, jcb, n, BLOCK=_BLOCK)
    return mu.reshape(shape), var.reshape(shape), jcb.reshape(shape)


def _innovation(y, mu, var, var_v):
    """Repo's normalized observation innovation (Triton). Returns (dmu, dvar)."""
    shape = mu.shape
    y, mu, var = y.reshape(-1), mu.reshape(-1), var.reshape(-1)
    dmu, dvar = torch.empty_like(mu), torch.empty_like(mu)
    n = mu.numel()
    _innovation_kernel[_grid(n)](y, mu, var, dmu, dvar, float(var_v), _DIV_EPS, n, BLOCK=_BLOCK)
...
class GaussianTensor:
    ...
    def __init__(self, mu, var, parents=(), op=None, name="", device=None):
        dtype = torch.get_default_dtype()
        mu_t = torch.as_tensor(mu, dtype=dtype)
        # Place moments on the GPU (the library's Triton kernels require it).
        # Intermediate op outputs are already device tensors, so keep their
        # device; raw leaf data (python lists / floats) goes to the default.
        if device is None:
            device = mu_t.device if isinstance(mu, Tensor) else _DEFAULT_DEVICE
        device = torch.device(device)
        self.mu = mu_t.to(device)
        self.var = torch.as_tensor(var, dtype=dtype).to(device)
    ...
    def observe(
        ...
        # Normalized innovation (Triton): dmu = (y-mu)/(var+var_v), dvar = -1/(...).
        delta_mu, delta_var = _innovation(y, self.mu, self.var, var_v)
...
def tensor(mu, var=0.0, name="x", device=None) -> GaussianTensor:
    """Wrap data (e.g. deterministic covariates: ``var=0``) as a graph leaf.

    Placed on ``device`` (defaults to the module device, i.e. the GPU) so the
    downstream Triton kernels can consume it.
    """
    return GaussianTensor(mu, var, name=name, device=device)
...
class Add(Operation):
    def forward(self, x: GaussianTensor, y: GaussianTensor) -> GaussianTensor:
        ...
        mu, var = _gauss_add(x.mu, x.var, y.mu, y.var)
        return GaussianTensor(mu, var, parents=(x, y), op=self, name="add")
...
class Mul(Operation):
    def forward(self, x: GaussianTensor, y: GaussianTensor) -> GaussianTensor:
        ...
        mu, var = _gauss_mul(x.mu, x.var, y.mu, y.var)
        return GaussianTensor(mu, var, parents=(x, y), op=self, name="mul")

    def backward(self, node: GaussianTensor) -> None:
        x, y = node.parents
        # delta_mu_X = mu_Y·delta_mu_Z, delta_var_X = mu_Y²·delta_var_Z (and swap).
        dmx, dvx = _scale_delta(y.mu, node.d_mu, node.d_var)
        dmy, dvy = _scale_delta(x.mu, node.d_mu, node.d_var)
        x._accumulate(dmx, dvx)
        y._accumulate(dmy, dvy)
...
class Linear(Module):
    ...
    def __init__(
        ...
        self.has_bias = bias
        self.name = name
        device = _DEFAULT_DEVICE if device is None else torch.device(device)
        self.rng = rng  # accepted for API compatibility (library init is unseeded)
        ...
        self._layer = _LinearLayer(
            ...
            gain_w=gain_w, gain_b=gain_b, bias=bias,
        )
